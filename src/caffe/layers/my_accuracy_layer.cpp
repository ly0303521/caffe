      ...
      if (num_better_predictions < top_k_) {
        ...
        if (top.size() > 1) ++top[1]->mutable_cpu_data()[label_value];
          ...
          for (int k = 0; k < num_labels; ++k) {
              num_predict = (bottom_data[i * dim + num_predict * inner_num_ + j] > bottom_data[i * dim + k * inner_num_ + j]) ? num_predict : k ;
          }
          LOG(INFO) << "outer_num_ " << i << ", predict_num " << num_predict << ", label_value " << label_value;
      }
      ++count;
    }
  }
